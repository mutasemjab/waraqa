        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h4>{{ __('messages.Edit_Book_Request') }}</h4>
                </div>
                <div class="card-body">
                    @if($errors->any())
                    ...
                    <form action="{{ route('bookRequests.update', $bookRequest->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="product_id" id="product_id" class="form-select @error('product_id') is-invalid @enderror" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $bookRequest->product_id == $product->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="provider_id" id="provider_id" class="form-select @error('provider_id') is-invalid @enderror" required>
                                ...
                                @foreach($providers as $provider)
                                    <option value="{{ $provider->id }}" {{ $bookRequest->provider_id == $provider->id ? 'selected' : '' }}>
                                        {{ $provider->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="requested_quantity" id="requested_quantity"
                                   ...
                                   value="{{ $bookRequest->requested_quantity }}" min="1" required>
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('bookRequests.show', $bookRequest->id) }}" class="btn btn-secondary">
                                {{ __('messages.Cancel') }}
                            </a>
                        </div>
                    </form>
