...
foreach ($prefixes as $prefix) {
    $keysFile = __DIR__ . '/' . $prefix . '_app.txt';

    // Skip if keys file doesn't exist
    if (!file_exists($keysFile)) {
        echo "$prefix" . "_app.txt not found, skipping...\n";
        continue;
    }
    ...
    echo "$prefix (from app):\n";
    echo "  - Total keys: " . count($keys) . "\n";
    echo "  - Missing in English: " . count($missingEn) . " keys" . (!empty($missingEn) ? " (saved to $prefix" . "_app/en.txt)" : "") . "\n";
    echo "  - Missing in Arabic: " . count($missingAr) . " keys" . (!empty($missingAr) ? " (saved to $prefix" . "_app/ar.txt)" : "") . "\n";
    echo "\n";
}
